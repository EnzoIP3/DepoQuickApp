...
export class BusinessPageComponent implements OnInit, OnDestroy {
  ...
  private _fetchBusinesses(paginationParams: Pagination = { page: 1, pageSize: 10 }): void {
    if (!this._userId) {
      return;
    }
    ...
    this._businessesSubscription = this._userService
      ...
      .subscribe({
        next: (response) => {
          this.businesses = response.businesses.filter((business) => business.rut === this._businessId);
